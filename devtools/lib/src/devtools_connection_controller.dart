...
class DevToolsConnectionController {
      ...
      if (localStream == null) {
        if (request.useExternalVideoTrack) {
          ...
          if (request.configuredVideo) {
            localStream.addTrack(_createExternalVideoTrack());
          }
        ...
        if (request.useExternalVideoTrack && request.configuredVideo) {
          // 再接続時は古い映像入力を破棄し、選択中の映像トラックを生成する。
  ...
  LocalVideoTrack _createExternalVideoTrack() {
    return MediaDevices.createExternalVideoTrack();
  }
