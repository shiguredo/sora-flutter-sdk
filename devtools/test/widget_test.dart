...
void main() {
  ...
  testWidgets('接続確認には実際に生成する主要設定を表示する', (WidgetTester tester) async {
    ...
    expect(find.text('video_source: camera'), findsOneWidget);
  ...
  testWidgets('ビットレート入力欄の単位を kbps で表示する', (WidgetTester tester) async {
    await tester.pumpWidget(
      const _ConnectionSettingsHarness(hasRetainedConnection: false),
    );

    expect(find.text('Audio Bitrate (kbps)'), findsOneWidget);
    expect(find.text('Video Bitrate (kbps)'), findsOneWidget);
  });
}
...
void _verifyInputValidation() {
  ...
  expect(validateOptionalAudioBitRate(''), isNull);
  expect(validateOptionalAudioBitRate('6'), isNull);
  expect(validateOptionalAudioBitRate('510'), isNull);
  expect(validateOptionalAudioBitRate('5'), isNotNull);
  expect(validateOptionalAudioBitRate('511'), isNotNull);
  expect(validateOptionalAudioBitRate('64000'), isNotNull);
