...
static uint8_t sora_clamp_u8(int value) {
  if (value < 0) {
    return 0;
  }
  if (value > 255) {
...
/* ReplayKit の BGRA フレームを BT.601 limited range の I420 へ変換する。 */
__attribute__((visibility("default"))) int sora_bgra_to_i420(
    ...
    int32_t width,
    int32_t height) {
  if (src_bgra == NULL || dst_y == NULL || dst_u == NULL || dst_v == NULL ||
      width <= 0 || height <= 0 || src_stride_bgra < width * 4 ||
      dst_stride_y < width || dst_stride_u < (width + 1) / 2 ||
  ...
  for (int32_t y = 0; y < height; ++y) {
    ...
    for (int32_t x = 0; x < width; ++x) {
      ...
      dst_row[x] =
          sora_clamp_u8(((66 * red + 129 * green + 25 * blue + 128) >> 8) + 16);
    }
  }

  for (int32_t y = 0; y < height; y += 2) {
    uint8_t* dst_u_row = dst_u + (y / 2) * dst_stride_u;
    uint8_t* dst_v_row = dst_v + (y / 2) * dst_stride_v;
    for (int32_t x = 0; x < width; x += 2) {
      int red_sum = 0;
      int green_sum = 0;
      int blue_sum = 0;
      int sample_count = 0;
      for (int32_t offset_y = 0; offset_y < 2 && y + offset_y < height;
           ++offset_y) {
        const uint8_t* src_row = src_bgra + (y + offset_y) * src_stride_bgra;
        for (int32_t offset_x = 0; offset_x < 2 && x + offset_x < width;
             ++offset_x) {
          const uint8_t* pixel = src_row + (x + offset_x) * 4;
          blue_sum += pixel[0];
          green_sum += pixel[1];
          red_sum += pixel[2];
          ++sample_count;
      ...
      dst_u_row[x / 2] = sora_clamp_u8(
          ((-38 * red - 74 * green + 112 * blue + 128) >> 8) + 128);
      dst_v_row[x / 2] = sora_clamp_u8(
...
static uint8_t sora_full_range_luma_to_limited(uint8_t value) {
  return sora_clamp_u8(((int)value * 219 + 127) / 255 + 16);
}

static uint8_t sora_full_range_chroma_to_limited(uint8_t value) {
  return sora_clamp_u8(((int)value * 224 + 127) / 255 + 16);
}

/* ReplayKit の FullRange NV12 を BT.601 limited range の I420 へ変換する。 */
__attribute__((visibility("default"))) int sora_full_range_nv12_to_i420(
    const uint8_t* src_y,
  ...
  if (src_y == NULL || src_uv == NULL || dst_y == NULL || dst_u == NULL ||
      dst_v == NULL || width <= 0 || height <= 0 || src_stride_y < width ||
      src_stride_uv < chroma_width * 2 || dst_stride_y < width ||
      dst_stride_u < chroma_width || dst_stride_v < chroma_width) {
    return -1;
  }

  for (int32_t y = 0; y < height; ++y) {
    const uint8_t* src_row = src_y + y * src_stride_y;
    uint8_t* dst_row = dst_y + y * dst_stride_y;
    for (int32_t x = 0; x < width; ++x) {
      dst_row[x] = sora_full_range_luma_to_limited(src_row[x]);
    }
  }

  for (int32_t y = 0; y < chroma_height; ++y) {
    const uint8_t* src_row = src_uv + y * src_stride_uv;
    uint8_t* dst_u_row = dst_u + y * dst_stride_u;
    uint8_t* dst_v_row = dst_v + y * dst_stride_v;
    for (int32_t x = 0; x < chroma_width; ++x) {
      dst_u_row[x] = sora_full_range_chroma_to_limited(src_row[x * 2]);
      dst_v_row[x] = sora_full_range_chroma_to_limited(src_row[x * 2 + 1]);
    }
  }
  return 0;
}
...
const int sora_audio_session_error_none = 0;
const int sora_audio_session_error_configuration = -1;
const int sora_audio_session_error_input_initialization = -2;

typedef struct SoraAudioSessionInitializeContext {
  sora_audio_session_initialize_input_callback callback;
  void* user_data;
} SoraAudioSessionInitializeContext;

static objc_AVAudioSessionMode sora_audio_session_mode(int prefers_video_mode) {
  return prefers_video_mode != 0 ? objc_AVAudioSessionMode_VideoChat()
                                 : objc_AVAudioSessionMode_VoiceChat();
}

/*
 * RTCAudioSession / AVAudioSession の設定はメインスレッド前提で扱う。
 * Swift 側は専用 queue からこの関数群を呼ぶため、実際の設定処理だけを
 * メインスレッドへ閉じ込める。
 */
static int sora_audio_session_configure_on_main_thread(int prefers_video_mode) {
  struct webrtc_objc_RTCAudioSession* session =
      webrtc_objc_RTCAudioSession_sharedInstance();
  if (session == NULL) {
    return sora_audio_session_error_configuration;
  }

  struct webrtc_objc_RTCAudioSessionConfiguration* config =
      webrtc_objc_RTCAudioSessionConfiguration_webRTCConfiguration();
  if (config == NULL) {
    webrtc_objc_RTCAudioSession_release(session);
    return sora_audio_session_error_configuration;
  }

  webrtc_objc_RTCAudioSessionConfiguration_setWebRTCConfiguration(config);
  webrtc_objc_RTCAudioSessionConfiguration_setCategory(
      config, objc_AVAudioSessionCategory_PlayAndRecord());
  webrtc_objc_RTCAudioSessionConfiguration_setMode(
      config, sora_audio_session_mode(prefers_video_mode));
  webrtc_objc_RTCAudioSessionConfiguration_setCategoryOptions(
      config, objc_AVAudioSessionCategoryOption_AllowBluetoothHFP() |
                  objc_AVAudioSessionCategoryOption_AllowBluetoothA2DP() |
                  objc_AVAudioSessionCategoryOption_DefaultToSpeaker());

  struct objc_NSError* error = NULL;
  webrtc_objc_RTCAudioSession_lockForConfiguration(session);
  int success = webrtc_objc_RTCAudioSession_setConfiguration_active_error(
      session, config, 1, &error);
  webrtc_objc_RTCAudioSession_unlockForConfiguration(session);

  if (error != NULL) {
    objc_NSError_release(error);
  }
  webrtc_objc_RTCAudioSessionConfiguration_release(config);
  webrtc_objc_RTCAudioSession_release(session);

  return success != 0 ? sora_audio_session_error_none
                      : sora_audio_session_error_configuration;
}

static int sora_audio_session_deactivate_on_main_thread(void) {
  struct webrtc_objc_RTCAudioSession* session =
      webrtc_objc_RTCAudioSession_sharedInstance();
  if (session == NULL) {
    return sora_audio_session_error_configuration;
  ...
  struct objc_NSError* error = NULL;
  webrtc_objc_RTCAudioSession_lockForConfiguration(session);
  int success = webrtc_objc_RTCAudioSession_setActive_error(session, 0, &error);
  webrtc_objc_RTCAudioSession_unlockForConfiguration(session);
...
static void sora_audio_session_on_initialize_input(struct objc_NSError* error,
                                                   void* user_data) {
  SoraAudioSessionInitializeContext* context =
      (SoraAudioSessionInitializeContext*)user_data;
  if (context == NULL) {
    if (error != NULL) {
      objc_NSError_release(error);
    }
  ...
  /* この実装は RTCAudioSession.initializeInput: の完了ハンドラが
       必ず1回だけ呼ばれる、という前提に依存している。
       0 回だと context がリークし、2 回以上だと free(context) の
       二重解放になる。 */

  int result = error == NULL ? sora_audio_session_error_none
                             : sora_audio_session_error_input_initialization;
  if (error != NULL) {
    objc_NSError_release(error);
  }
...
__attribute__((visibility("default"))) int sora_audio_session_configure(
    int prefers_video_mode) {
  if (pthread_main_np() != 0) {
    return sora_audio_session_configure_on_main_thread(prefers_video_mode);
  }

  __block int result = sora_audio_session_error_configuration;
  dispatch_sync(dispatch_get_main_queue(), ^{
    result = sora_audio_session_configure_on_main_thread(prefers_video_mode);
  });
  return result;
}
...
sora_audio_session_initialize_input_async(
  ...
  if (session == NULL) {
    if (callback != NULL) {
      callback(sora_audio_session_error_input_initialization, user_data);
    }
    return;
  }

  SoraAudioSessionInitializeContext* context =
      (SoraAudioSessionInitializeContext*)calloc(1, sizeof(*context));
  if (context == NULL) {
    webrtc_objc_RTCAudioSession_release(session);
    if (callback != NULL) {
      callback(sora_audio_session_error_input_initialization, user_data);
  ...
  /* webrtc_objc_RTCAudioSession_initializeInput の完了ハンドラが
       必ず1回だけ呼ばれる、前提に依存する。
       早期失敗パス (session==NULL, calloc 失敗) は自前でコールバック必達に
       しているが、dispatch_async 委譲後は upstream 任せになる。 */
  dispatch_async(dispatch_get_main_queue(), ^{
    webrtc_objc_RTCAudioSession_initializeInput(
        session, sora_audio_session_on_initialize_input, context);
    webrtc_objc_RTCAudioSession_release(session);
  });
}

__attribute__((visibility("default"))) int sora_audio_session_deactivate(void) {
  if (pthread_main_np() != 0) {
    return sora_audio_session_deactivate_on_main_thread();
  }

  __block int result = sora_audio_session_error_configuration;
  dispatch_sync(dispatch_get_main_queue(), ^{
    result = sora_audio_session_deactivate_on_main_thread();
  });
  return result;
}
#endif
...
static void on_apple_discarded(void* user_data) {
  (void)user_data;
}
static void on_apple_destroy(void* user_data) {
  (void)user_data;
}
...
__attribute__((visibility("default"))) AppleRenderingSink*
apple_rendering_sink_create(void) {
...
__attribute__((visibility("default"))) void
apple_rendering_sink_set_frame_callback(AppleRenderingSink* sink,
...
__attribute__((visibility("default"))) void*
apple_rendering_sink_copy_pixel_buffer(AppleRenderingSink* sink) {
  if (sink == NULL)
    return NULL;
  ...
  if (sink->pixel_buffer == NULL) {
    ...
    if (empty_dict != NULL) {
      ...
      attrs = CFDictionaryCreate(NULL, (const void**)keys, (const void**)values,
    ...
    CVReturn status = CVPixelBufferCreate(NULL, (size_t)w, (size_t)h,
  ...
  if (pixels != NULL) {
    ...
    libyuv_ConvertFromI420(webrtc_I420Buffer_MutableDataY(sink->i420_buffer),
                           ...
                           (uint8_t*)pixels, pitch, w, h, libyuv_FOURCC_ARGB);
  }
...
__attribute__((visibility("default"))) void apple_rendering_sink_delete(
    AppleRenderingSink* sink) {
  if (sink == NULL)
    return;
...
sora_observer_bridge_create(dart_on_state_fn on_connection_change,
                            dart_on_state_fn on_ice_connection_change,
                            dart_on_state_fn on_ice_gathering_change,
                            dart_on_ice_candidate_fn on_ice_candidate,
                            dart_on_track_fn on_track,
...
sora_observer_bridge_get_observer(SoraObserverBridge* bridge) {
...
__attribute__((visibility("default"))) void* apple_rendering_sink_get_sink_ptr(
    AppleRenderingSink* sink) {
  if (sink == NULL)
    return NULL;
