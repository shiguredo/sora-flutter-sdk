...
void main() {
  ...
  group('SoraLocalVideoWidget', () {
    ...
    testWidgets('textureId が -1 のときプレースホルダーが表示される', (tester) async {
      // 呼び出し側から誤って負値が渡されても Flutter の Texture ウィジェット
      // に無効な id を流して黒画面にならないよう防御することを検証する。
      await pumpLocalWidget(
        tester,
        textureId: -1,
        placeholder: const Text('no video'),
    ...
    testWidgets('textureId が 0 のとき Texture が存在する', (tester) async {
      // 0 は有効な texture id として Texture を描画し、負値ガードで
      // 巻き添えにしないことを検証する。
      await pumpLocalWidget(tester, textureId: 0);
      expect(find.byType(Texture), findsOneWidget);
    });
  });
}
