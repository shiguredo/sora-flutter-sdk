...
- 機密情報の実値そのものを残さないこと
  - 対象: API キー・アクセストークン・シークレット・パスワード・秘密鍵、接続文字列・認証情報・内部エンドポイントの URL やホスト名、顧客やユーザーの個人情報（メールアドレス・氏名・電話番号・IP アドレス等）、社外秘の情報
  - 対象範囲: issue・ソースコード・コメント・コミットメッセージ・CHANGES.md・PR の本文など、リポジトリに残るすべての成果物
- 実値を一部だけ伏せて残すのではなく、そもそも実値を書かないこと
- 具体的な値が必要に見える場合は、実値ではなくプレースホルダ（`<API_KEY>` 等）や参照（環境変数名・シークレット管理の場所・設定ファイルのキー名）で示すこと
- 入力に機密情報が含まれていても、それを成果物に書き写さないこと
...
- Issue を作成した LLM の Model と Version をファイルのタイトルの後に `Model: <model-name> <version>` として記載すること
  - Opus 4.8 や GPT-5.5 や DeepSeek V4 Pro など
...
- コメントの日本語は固有名詞や仕様用語まで厳密に日本語にしようとしないこと
- 正式リリースまでは CHANGES.md に反映しない
