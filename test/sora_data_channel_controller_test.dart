...
void main() {
  ...
  group('signaling close の reason 型防御', () {
    // signaling close メッセージの reason は JSON 由来で Object? のため、
    // 数値や Map など String 以外の値が来ても `as String?` の同期 TypeError で
    // silent drop されず onSignalingClose が呼ばれることを検証する。
    Future<
      ({List<Object?> codes, List<String?> reasons, List<String> debugMessages})
    >
    runCloseCase({required Object? rawReason}) async {
      final codes = <Object?>[];
      ...
      final ctrl = DataChannelController(
        ...
        onConnectionCreated: (_) {},
        // 実 JSON decode の副作用を避けて payload をそのまま返し、
        // reason の型が Object? のまま _handleSignalingDataChannelMessage
        // に渡る経路を再現する。
        decodeJsonMap: (text) async {
      ...
      try {
        await ctrl.handleMessage('signaling', '{"stub":"payload"}');
        return (codes: codes, reasons: reasons, debugMessages: debugMessages);
      } finally {
        wc.dispose();
      }
    }
    ...
    test(
      'reason が数値でも onSignalingClose が code=4200 / reason=null で呼ばれる',
      () async {
        final result = await runCloseCase(rawReason: 4200);

        expect(result.codes, hasLength(1));
        expect(result.codes.single, 4200);
        expect(result.reasons, [null]);
        expect(
          result.debugMessages.any(
            (m) => m.contains('unexpected reason type') && m.contains('int'),
          ),
          isTrue,
          reason: 'reason 型が想定外の場合にデバッグメッセージが出ること',
        );
      },
    );

    test('reason が Map でも onSignalingClose が reason=null で呼ばれる', () async {
      final result = await runCloseCase(
        rawReason: <String, Object?>{'nested': 'value'},
      );

      expect(result.codes, hasLength(1));
      expect(result.reasons, [null]);
      expect(
        result.debugMessages.any((m) => m.contains('unexpected reason type')),
        isTrue,
      );
    });

    test('reason が正常な String の場合はそのまま onSignalingClose に渡り警告が出ない', () async {
      final result = await runCloseCase(rawReason: 'bye');

      expect(result.codes, hasLength(1));
      expect(result.reasons, ['bye']);
      expect(
        result.debugMessages.any((m) => m.contains('unexpected reason type')),
        isFalse,
      );
    ...
    test('reason が null の場合はそのまま null で渡り警告が出ない', () async {
      final result = await runCloseCase(rawReason: null);
