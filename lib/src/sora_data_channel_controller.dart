  ...
  Future<void> _handleSignalingDataChannelMessage(Object? data) async {
    ...
    try {
      ...
      if (type == 're-offer') {
        ...
        // decoded['reason'] は JSON 由来で Object?。String 以外の値を
        // `as String?` でキャストすると同期 TypeError になり外側 catch で
        // 握りつぶされて onSignalingClose が呼ばれなくなる。is String で
        // 型防御し、String または null に正規化する。
        final rawReason = decoded['reason'];
        final reason = rawReason is String ? rawReason : null;
        if (rawReason != null && rawReason is! String) {
          onDebugMessage(
            'dc(signaling) close message contains unexpected reason type: '
            '${rawReason.runtimeType}',
          );
        }
