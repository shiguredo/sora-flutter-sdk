...
void main() {
  ...
  group('ignoreDisconnectWebSocket', () {
    test('デフォルト値は false である', () {
      final state = SignalingSessionState();
      expect(state.ignoreDisconnectWebSocket, false);
    });

    test('resetSession() で false にリセットされる', () {
      final state = SignalingSessionState();
      state.ignoreDisconnectWebSocket = true;
      expect(state.ignoreDisconnectWebSocket, true);
      state.resetSession();
      expect(state.ignoreDisconnectWebSocket, false);
    });
  });
