...
linux_rendering_sink_copy_pixels(LinuxRenderingSink* sink,
  ...
  if (sink->i420_buffer == NULL) {
    if (sink->rgba_buffer == NULL) {
      ...
      sink->rgba_buffer_size = 16;
      sink->width = 2;
      sink->height = 2;
    }
    *out_width = (uint32_t)sink->width;
    *out_height = (uint32_t)sink->height;
...
// Observer ブリッジ
// ===========================================================================
// PeerConnectionObserver コールバックと DataChannelObserver コールバックを
// C 側で安全に処理し、Dart 側の NativeCallable.listener へ中継する。
...
// Observer の OnDestroy では何もしない。
// 実際の破棄は sora_observer_bridge_destroy で行う。
static void bridge_noop_destroy(void* user_data) {
  (void)user_data;
}
...
sora_observer_bridge_create(void* on_connection_change,
  ...
  bridge->on_datachannel = (dart_on_datachannel_fn)on_datachannel;
  bridge->on_debug = (dart_on_debug_fn)on_debug;
  ...
  obs_cbs.OnDestroy = bridge_noop_destroy;
...
sora_observer_bridge_setup_dc(SoraObserverBridge* bridge,
  ...
  ctx->bridge = bridge;
  ctx->dc = dc;
  ctx->on_state_change = (dart_on_dc_state_fn)on_state_change;
  ctx->on_message = (dart_on_dc_message_fn)on_message;
  ctx->dart_user_data = dc_user_data;
  struct std_string_unique* label = webrtc_DataChannelInterface_label(dc);
  const char* label_cstr = std_string_c_str(std_string_unique_get(label));
  ...
  cbs.OnDestroy = bridge_noop_destroy;
  struct webrtc_DataChannelObserver* observer =
...
// SoraObserverBridge を破棄し、observer の削除と解放を行う。
