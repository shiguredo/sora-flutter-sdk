...
// compress=true / false の両方で #test-channel の open を確認し、
// バイナリメッセージの送受信が成功することを確認する。また、
...
void main() {
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      await _runCustomDataChannelTest(
        tester,
        compress: true,
        channelSuffix: '-custom-dc',
      );
    },
  );

  testWidgets(
    'custom_data_channel_compress_off: #test-channel の open と'
    'compress=false のメッセージ送受信を検証する',
    (WidgetTester tester) async {
      await _runCustomDataChannelTest(
        tester,
        compress: false,
        channelSuffix: '-custom-dc-compress-off',
      );
    },
  );
}

/// 指定した compress 設定で custom DataChannel の open と送受信を検証する。
Future<void> _runCustomDataChannelTest(
  WidgetTester tester, {
  ...
  final senderConfig = SoraConnectionConfig(
    ...
    dataChannels: <Map<String, Object?>>[
      {'label': testLabel, 'direction': 'sendrecv', 'compress': compress},
    ],
