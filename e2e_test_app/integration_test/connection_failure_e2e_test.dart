...
void main() {
  ...
  testWidgets(
    'connection_failure: DataChannel signaling 切替後の WebSocket 切断で'
    '接続が維持されることを確認する',
    (WidgetTester tester) async {
      // ignore_disconnect_websocket: true では switched 後に WebSocket が
      // 閉じられても、WebRTC 接続は維持されなければならない。
      // 検証環境の Sora サーバーは、switched 送信後に WebSocket を閉じる
      // 必要がある。
      final env = loadE2eEnvironment();
      final channelId = buildChannelId(env.channelPrefix, suffix: '-ignore-ws');
      ...
      try {
        logE2eMessage('stage=connect_start type=ignore_disconnect_websocket');
        await conn.connect();
        await conn.waitUntilConnected(timeout);
        conn.throwIfHasErrors();
        logE2eMessage('stage=connected type=ignore_disconnect_websocket');

        logE2eMessage('stage=wait_switched type=ignore_disconnect_websocket');
        await conn.waitForSwitched(tester, timeout: timeout);

        // switched 後に WebSocket が閉じられても接続が維持されることを
        // 一定時間の経過と getStats の成功で確認する。
        await tester.pump(const Duration(seconds: 5));
        expect(
          conn.milestones.contains('SoraDisconnectedState'),
          isFalse,
          reason: 'switched 後に SoraDisconnectedState が発火しないこと。',
        );

        final stats = await conn.connection.getStats();
        expect(stats, isNotNull, reason: 'switched 後も getStats が成功すること。');
        conn.throwIfHasErrors();
        logE2eMessage('stage=connection_kept type=ignore_disconnect_websocket');

        await conn.disconnect();
        await conn.waitUntilDisconnected(timeout);
        logE2eMessage('stage=disconnected type=ignore_disconnect_websocket');
      } finally {
        logE2eMessage('stage=cleanup type=ignore_disconnect_websocket');
        await conn.dispose();
      }
    },
  );
