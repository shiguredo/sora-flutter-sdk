...
`.github/workflows/e2e-test.yml` は `workflow_dispatch` のみです。Actions 画面から手動実行し、**Repository secrets** に `TEST_SECRET_KEY` / `TEST_SIGNALING_URLS` / `TEST_CHANNEL_ID_PREFIX` を登録してください。
