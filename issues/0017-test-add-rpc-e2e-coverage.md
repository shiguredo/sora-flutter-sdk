...
2. Flutter には JWT 生成機能がないため、テストコード内で HS256 JWT を生成する実装が必要。既に transitive 依存で導入済みの `crypto` パッケージを利用し (`e2e_test_app` の dev_dependencies へ直接依存として明示)、Hmac 署名で JWT を生成する。署名鍵は既存の `TEST_SECRET_KEY` を使い、`buildChannelId` で生成したチャンネル ID を payload の `channel_id` に埋め込む。生成した JWT は `metadata` に直接設定する
...
- 次の環境変数を `e2e_test_app/README.md` に定義し、テストコードへベタ書きしない
  - `TEST_SECRET_KEY`: HS256 JWT の署名鍵（既存の E2E 用シークレットを流用する）
...
- スキップ判定: `TEST_SECRET_KEY` または `TEST_RPC_SUCCESS_METHOD` が未設定の場合は接続前にスキップする。設定済みでも offer の `rpc_methods` にテスト対象メソッドが含まれない場合はスキップする (iOS SDK の offer 判定パターン)
...
- 前提条件 (`TEST_SECRET_KEY` と `TEST_RPC_*` の設定、JWT private claims による `rpc_methods` 払い出し) を満たさない環境ではテストがスキップされること
...
2. 既に transitive 依存で導入済みの `crypto` パッケージを `e2e_test_app` の dev_dependencies へ直接依存として明示し、テストコード内で HS256 JWT を生成する。署名鍵は `TEST_SECRET_KEY` を使い、payload には `channel_id` と private claims の `rpc_methods` を含め、生成した JWT を metadata に設定して接続する
