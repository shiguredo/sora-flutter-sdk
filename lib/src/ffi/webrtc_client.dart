...
class WebrtcClient {
  ...
  //
  // MediaEngine は全 PeerConnection の破棄で Terminate し、次の作成で再び
  // Init される。Init のたびに 1 と 2 が再適用される必要があるため、
  // PeerConnection 作成ごとに本補正を呼ぶ。呼び出しは冪等ではなく、既に
  // 適用済みの場合は ADM 側が -1 を返すだけで実害はない。
  //
